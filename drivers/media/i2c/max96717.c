...
static int max96717_apply_patgen(struct max96717_priv *priv)
{
	...
	if (priv->pattern)
		ret = max96717_apply_patgen_timing(priv);
				   ...
				   u64 streams_mask)
{
	...
	ret = max96717_apply_patgen(priv);
	if (ret)
		goto stop_csi;
...
static int max96717_s_power(struct v4l2_subdev *sd, int on)
{
	return 0;
}
...
static int max96717_link_setup(struct media_entity *entity,
...
static const struct media_entity_operations max96717_entity_ops = {
	...
	.link_setup = max96717_link_setup,
};
