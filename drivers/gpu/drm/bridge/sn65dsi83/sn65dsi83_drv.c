...
static int sn65dsi83_get_modes(struct sn65dsi83 *sn65dsi83,
...
{
	return sn65dsi83_get_modes(connector_to_sn65dsi83(connector),
			connector);
}
...
static void sn65dsi83_bridge_enable(struct drm_bridge *bridge)
{
	...
	if (sn65dsi83->brg->backlight) {
		sn65dsi83->brg->backlight->props.state &= ~BL_CORE_FBBLANK;
		sn65dsi83->brg->backlight->props.power = FB_BLANK_UNBLANK;
		backlight_update_status(sn65dsi83->brg->backlight);
	}
}
...
static void sn65dsi83_bridge_disable(struct drm_bridge *bridge)
{
	...
	if (sn65dsi83->brg->backlight) {
		sn65dsi83->brg->backlight->props.power = FB_BLANK_POWERDOWN;
		sn65dsi83->brg->backlight->props.state |= BL_CORE_FBBLANK;
		backlight_update_status(sn65dsi83->brg->backlight);
	}
		...
		enum drm_bridge_attach_flags flags)
{
	...
	drm_connector_helper_add(&sn65dsi83->connector,
		...
		drm_connector_attach_encoder(&sn65dsi83->connector,
				bridge->encoder);
	}
...
static int sn65dsi83_bridge_get_modes(struct drm_bridge *bridge,
		struct drm_connector *connector)
{
	return sn65dsi83_get_modes(bridge_to_sn65dsi83(bridge),
			connector);
}
    ...
    struct sn65dsi83 *sn65dsi83)
{
	...
	sn65dsi83->brg->gpio_enable = devm_gpiod_get_optional(dev, "enable", GPIOD_OUT_LOW);
	if (IS_ERR(sn65dsi83->brg->gpio_enable)) {
		dev_err(dev, "failed to parse enable gpio");
		return PTR_ERR(sn65dsi83->brg->gpio_enable);
	}

	sn65dsi83->brg->gpio_panel_enable = devm_gpiod_get_optional(dev, "enable-panel", GPIOD_OUT_LOW);
	if (IS_ERR(sn65dsi83->brg->gpio_panel_enable)) {
		dev_err(dev, "failed to parse enable panel gpio");
		return PTR_ERR(sn65dsi83->brg->gpio_panel_enable);
	}
	else {
		if (sn65dsi83->brg->gpio_panel_enable) {
			gpiod_set_value_cansleep(sn65dsi83->brg->gpio_panel_enable, 0);
			msleep(200);
		}
	}

	sn65dsi83->brg->backlight = devm_of_find_backlight(dev);
	if (IS_ERR(sn65dsi83->brg->backlight)) {
		sn65dsi83->brg->backlight = NULL;
		return -EPROBE_DEFER;;
	}
	...
	if ((of_get_videomode(np, &sn65dsi83->brg->vm, 0)) < 0) {
		dev_err(dev, "failed to parse mandatory 'display-timings' node");
		return -EINVAL;
	}

	sn65dsi83->brg->test_pattern = of_property_read_bool(np, "test-pattern");
	dev_dbg(dev, "test-pattern = %s \n", sn65dsi83->brg->test_pattern ? "yes" : "no");
...
static int sn65dsi83_probe(struct i2c_client *i2c)
{
	...
	sn65dsi83->bridge.type = DRM_MODE_CONNECTOR_LVDS;
	sn65dsi83->bridge.ops = DRM_BRIDGE_OP_MODES;

	dev_set_drvdata(dev, sn65dsi83);

	ret = sn65dsi83_brg_probe(sn65dsi83->brg);
	if (ret < 0) {
		dev_err(dev, "failed to probe sn65dsi83 bridge \n");
		return ret;
	}
...
static void sn65dsi83_remove(struct i2c_client *i2c)
{
	...
	sn65dsi83_brg_remove(sn65dsi83->brg);
